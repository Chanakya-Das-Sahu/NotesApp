...
const Edit = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            console.log(noteId);
            const res = await axios.get(`http://localhost:3000/note/get/${noteId}`, {
                headers: {
                    'Authorization': Token,
                }
            });
    ...
    const handleEdit = async () => {
        const res = await axios.put(`http://localhost:3000/note/edit/${noteId}`, data, {
            headers: {
                'Authorization': Token,
            }
        });
        ...
        <>
            <div className="border-4 border-[#7761a7] bg-teal-400 rounded-2xl w-[700px] h-[600px] flex flex-col absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 m-2">
                ...
                <textarea
                    type="text"
                    name="content"
                    className="border-4 border-[#7761a7] m-2 h-[530px] w-[675px] rounded-2xl p-2 resize-none text-2xl text-start"
                    onChange={(e) => { setData({ ...data, [e.target.name]: e.target.value }) }}
                    value={data.content}
                ></textarea>
